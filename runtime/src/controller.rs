...
impl Robot {
    ...
    pub async fn run(&self, model: Arc<Model>) -> Result<()> {
        ...
        loop {
            ...
            let desired_joint_positions =
                self.model_inference(&model, &current_joint_states).await?;
    ...
    async fn get_joint_states(&self) -> Result<[f32; 16]> {
        ...
        let joint_states: [f32; 16] = servo_data
            .servo
            .iter()
            .take(16)
    ...
    async fn model_inference(&self, model: &Model, joint_states: &[f32; 16]) -> Result<[f32; 16]> {
        ...
        let desired_joint_positions: [f32; 16] = model_output
            .iter()
            .take(16)
            .copied()
            .collect::<Vec<f32>>()
    ...
    async fn send_joint_commands(&self, positions: &[f32; 16]) -> Result<()> {
        ...
        for (i, &position) in positions.iter().enumerate().take(16) {
            cmd.ids[i] = (i + 1) as u8;
            cmd.positions[i] = position as i16;
            cmd.times[i] = 20;
        }
