...
class EulerPlotter:
    def __init__(self):
        ...
        self.euler_lines = [self.ax1.plot([], [], label=angle)[0] for angle in ['Roll', 'Pitch', 'Yaw']]
        self.ax1.legend()
        ...
        self.raw_lines_gyro = [self.ax2.plot([], [], label=f'Gyro {axis}')[0] for axis in ['X', 'Y', 'Z']]
        self.raw_lines_accel = [self.ax2.plot([], [], '--', label=f'Accel {axis}')[0] for axis in ['X', 'Y', 'Z']]
        self.ax2.legend()
    ...
    def update(self, euler_angles, gyro, accel, quaternions):
        ...
        return self.euler_lines + self.raw_lines_gyro + self.raw_lines_accel + self.quat_lines
...
if __name__ == "__main__":
    ...
    def animate(frame):
        ...
        raw_gyro = np.array([imu_data['gyro']['x'], imu_data['gyro']['y'], imu_data['gyro']['z']])
        raw_accel = np.array([imu_data['accel']['x'], imu_data['accel']['y'], imu_data['accel']['z']])
    ...
    ani = FuncAnimation(plotter.fig, animate, interval=10, blit=True, cache_frame_data=False)
    plt.show()
